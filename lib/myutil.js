...
var numberFiels = "";
var booleanFiels = "";
var dateFields = "create_time,update_time,birth,annual_inspect_date,change_date,valid_time,arrive_time,leave_time,last_arrive,evaluate_time,rcv_day,rcv_time,";
var arrayFields = "photo,params.air_time,order_detail,sku_info,answer,";
...
exports.getCreateJson = function(query, create_fields){
    var create_json = {};
    var create_fields = create_fields + ",";
    for (var key in query) {
        if (key != "fields" && key != "format" && key != "sorts" && key != "min_id" && key != "max_id" && key != "limit" && key != "access_token") {
            if (create_fields == "," || create_fields.indexOf(key + ",") > -1) {
                if (getFieldType(key) == "date") {
                    create_json[key] = new Date(query[key]);
                }
                if (getFieldType(key) == "array") {
                    create_json[key] = JSON.parse(query[key]);
                } else {
                    create_json[key] = query[key]
                }
            }
        }
    }
...
exports.getQueryJson = function(query, query_fields){
    var query_json = {};
    var query_fields = query_fields + ",";
    var has_query = false;
    for (var key in query) {
        if (key != "fields" && key != "format" && key != "sorts" && key != "min_id" && key != "max_id" && key != "limit" && key != "page" && key != "access_token") {
            var val = query[key];
            if (query_fields == "," || query_fields.indexOf(key + ",") > -1) {
                if (getFieldType(key) == "date") {
                    // 时间段 2015-11-01 00:00:00@2015-11-31 23:59:59
                    var p = val.indexOf("@");
                    if (p > -1) {
                        var begin_time = new Date(val.substr(0, p));
                        var end_time = new Date(val.substr(p + 1, val.length - p));
                        query_json[key] = {"$gte": begin_time, "$lte": end_time};
                    } else {
                        query_json[key] = new Date(val);
                    }
                } else {
                    if (val.indexOf("^") == 0) {  //模糊查询
                        val = val.substr(1, val.length - 1);
                        query_json[key] = {"$regex": val};
                    } else if (val.indexOf(">=") == 0){ //大于等于
                        val = parseInt(val.substr(2, val.length - 2));
                        query_json[key] = {"$gte": val};
                    } else if (val.indexOf("<=") == 0){ //小于等于
                        val = parseInt(val.substr(2, val.length - 2));
                        query_json[key] = {"$lte": val};
                    } else if (val.indexOf(">") == 0){  //大于
                        val = parseInt(val.substr(1, val.length - 1));
                        query_json[key] = {"$gt": val};
                    } else if (val.indexOf("<") == 0){  //小于
                        val = parseInt(val.substr(1, val.length - 1));
                        query_json[key] = {"$lt": val};
                    } else if (val.indexOf("|") > 0){
                        var arr = val.split("|");
                        for(var i = 0; i < arr.length; i++){
                            arr[i] = parseInt(arr[i]);
                        }
                        query_json[key] = {"$in": arr};
                    } else {
                        query_json[key] = val;
                    }
                }
...
exports.getQueryAndUpdateJson = function(query, query_fields, update_fields){
    var query_json = {};
    var update_json = {};
    ...
    for (var key in query) {
        if (key != "fields" && key != "format" && key != "sorts" && key != "min_id" && key != "max_id" && key != "limit" && key != "access_token") {
            if (key.indexOf("_") == 0) {  //查询字段
                ...
                if (update_fields == "," || update_fields.indexOf(key + ",") > -1) {
                    if (getFieldType(key) == "date") {
                        update_json[key] = new Date(query[key]);
                    }
                    if (getFieldType(key) == "array") {
                        update_json[key] = JSON.parse(query[key]);
                    } else {
                        update_json[key] = query[key]
                    }
                }
            }
        }
...
exports.getFieldJson = function(fields){
    var json = {};
    if(fields != ""){
...
exports.getSortJson = function(sorts){
    var json = {};
    if(sorts != ""){
        var arr = sorts.split(",");
        for (var i = 0; i < arr.length; i++) {
            if(arr[i].indexOf("-") == 0){
                var sort_field = arr[i].substr(1, arr[i].length - 1);
                json[sort_field] = -1;
            }else{
                json[arr[i]] = 1;
            }
...
exports.getID = function (id, key) {
    var _id = 0;
    if (typeof id == "undefined") {
        _id = 0;
    } else {
        _id = parseInt(id);
        if (_id != 0 && getFieldType(key) == "date") {
            _id = new Date(id);
        }
    }
    return _id;
...
function clone(myObj){
    if(typeof(myObj) != 'object' || myObj instanceof Date || myObj == null) return myObj;
...
// 推送逻辑
//极光消息推送
exports._sendJPushChat = function(cust_id, cust_name, msg_type, message, url, friend_id, voice_len, lon, lat, address, callback){
...
var sendWeixin = function(template_id, data, open_id, link, callback) {
    var data = encodeURIComponent(JSON.stringify(data));
    var url = "http://h5.bibibaba.cn/send_weixin.php?template_id=" + template_id + "&open_id=" + open_id +  "&url=" + link + "&data=" + data;
    _get(url, function(obj){
        callback(obj);
    });
};

exports.sendWeixin = sendWeixin;

exports.sendWeixinNewMsg = function(from, content, open_id, link, callback){
    var now = new Date();
    var template_id = "FB1J1WM7tYMFPe0dScOBRc0MmGaOA_2VnBaNE1hnzH4";
    var data = {
        "first": {
            "value": content,
            "color": "#173177"
    ...
    sendWeixin(template_id, data, open_id, link, callback);
};

exports.sendWeixinBuyNotify = function(product, remark, open_id, link, callback){
    var template_id = "TzlTbXYteI9RPLLrWll6Dz5P6sBB1UUlsY-vs6ucWYE";
    var data = {
        "name": {
            "value": product,
            "color": "#173177"
