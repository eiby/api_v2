...
var numberFiels = "";
var booleanFiels = "";
var dateFields = "create_time,update_time,birth,annual_inspect_date,change_date,valid_time,arrive_time,leave_time,last_arrive,evaluate_time,";
...
exports.getCreateJson = function(query, create_fields){
    ...
    for (var key in query) {
        if (key != "fields" && key != "format" && key != "sorts" && key != "min_id" && key != "max_id" && key != "limit") {
            if (create_fields.indexOf(key + ",") > -1) {
                if (getFieldType(key) == "date") {
...
exports.getQueryJson = function(query, query_fields){
    ...
    var has_query = false;
    for (var key in query) {
        if (key != "fields" && key != "format" && key != "sorts" && key != "min_id" && key != "max_id" && key != "limit" && key != "page") {
            var val = query[key];
            if (query_fields.indexOf(key + ",") > -1) {
                if (getFieldType(key) == "date") {
    ...
    for (var key in query) {
        if (key != "fields" && key != "format" && key != "sorts" && key != "min_id" && key != "max_id" && key != "limit") {
            if (key.indexOf("_") == 0) {  //查询字段
                ...
                if (update_fields.indexOf(key + ",") > -1) {
                    if (getFieldType(key) == "date") {
